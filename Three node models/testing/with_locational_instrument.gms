...
objective..                     WF =e= sum((t), A_zonal(t) * sum(n, LOAD_redi(t,n)) + 1/2 * S_zonal(t) * sum(n, LOAD_redi(t,n)) * sum(n, LOAD_redi(t,n)))
...
*p_ref * (1-(1/elasticity) + (sum((tec,n), GEN.L(t,tec,n)) / sum(n, elasticity * load_ref(t,n))));
...
network_cost = (sum((n,m),(GRID_CAP.L(n,m) / 2 * grid_cost(n,m))) + sum((t,tec,n), (UP.L(t,tec,n) - DOWN.L(t,tec,n)) * c_var(tec,n))
                ...
                - sum((t), A_zonal(t) * LOAD_spot.L(t) + 1/2 * S_zonal(t) * LOAD_spot.L(t) * LOAD_spot.L(t))
                );

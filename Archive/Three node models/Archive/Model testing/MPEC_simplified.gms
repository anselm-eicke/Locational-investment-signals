...
objective..                 WF =e= sum(t, p_ref * sum((tec,n), GEN(t,tec,n)) * (1-1/elasticity + sum((tec,n), GEN(t,tec,n)) / (2*elasticity* sum(n,load(t,n)))))
                                    ...
                                    - sum((t,tec,n), GEN(t,tec,n) * c_var(tec,n) + generation_slope * GEN(t,tec,n) * GEN(t,tec,n))
...
*KKT conditions
KKT_GEN(t,tec,n)..              c_var(tec,n) + 2 * generation_slope * GEN(t,tec,n) - p_ref * (1-(1/elasticity)) - p_ref * sum((ttec, nn), GEN(t,ttec,nn)) / (elasticity * sum(nn, load(t,nn))) + mu_G_max(t,tec,n) - mu_G_min(t,tec,n) =e= 0;
...
o_price(t) = p_ref * (1-(1/elasticity) + (sum((tec,n), GEN.L(t,tec,n)) / sum(n, elasticity * load(t,n))));
load_deviation(t) = sum((tec,n), GEN.L(t,tec,n)) / sum(n,load(t,n));

Display GEN.L, CAP.L, o_price, load_deviation, INSTRUMENT.L;
